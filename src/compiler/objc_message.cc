...
namespace google { namespace protobuf { namespace compiler { namespace objectivec {
  ...
  void MessageGenerator::GenerateBuilderSource(io::Printer* printer) {
    printer->Print(
      ...
      "@property (retain) $classname$* _builderResult;\n"
      "@end\n"
      "\n"
    ...
    printer->Print(
      "- (id) init {\n"
      "  if ((self = [super init])) {\n"
      "    self._builderResult = [[[$classname$ alloc] init] autorelease];\n"
      "  }\n"
      "  return self;\n"
      "}\n",
      "classname", ClassName(descriptor_));
  ...
  void MessageGenerator::GenerateCommonBuilderMethodsSource(io::Printer* printer) {
    ...
    printer->Print(
      "- ($classname$_Builder*) clear {\n"
      "  _builderResult = [[[$classname$ alloc] init] autorelease];\n"
      "  return self;\n"
      "}\n"
      "- ($classname$_Builder*) clone {\n"
      "  return [$classname$ builderWithPrototype:_builderResult];\n"
      "}\n"
      "- ($classname$*) defaultInstance {\n"
    ...
    printer->Outdent();
    printer->Print(
      "  $classname$* returnMe = [[_builderResult retain] autorelease];\n"
      "  self._builderResult = nil;\n"
      "  return returnMe;\n"
      "}\n",
      "classname", ClassName(descriptor_));
    ...
    for (int i = 0; i < descriptor_->field_count(); i++) {
      ...
      if (field->is_required()) {
        printer->Print(
          ...
          "  return NO;\n"
          "}\n",
          "capitalized_name", FieldName(field));
      }
    }
    ...
    for (int i = 0; i < descriptor_->field_count(); i++) {
      ...
      if (field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE &&
        HasRequiredFields(field->message_type())) {
          ...
          vars["capitalized_name"] = FieldName(field);
